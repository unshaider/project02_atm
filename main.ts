...
let pin: {enterPin: number} = await inquirer.prompt([
  {
    type: "password",
    name: "enterPin",
...
if (pin.enterPin != pinCode) {
  ...
  if (cash.selectOption === "Check Balance") {
    ...
    if (cashWithdrawl.withdraw > balance) {
      console.log(
        chalk.red("Your current balance is insufficient for this transaction.")
      );
    } else {
    ...
    const depositAmount: number = parseInt(amount.depositAmount);
